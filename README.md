# 🏆2024-2 한림대학교 소프트웨어캡스톤디자인_대상🏆
![2024-2 한림대학교 소프트웨어캡스톤디자인_대상](https://github.com/user-attachments/assets/09f63897-ea17-4769-8fb0-4490afd6c95d)

---

<br>

# 🧩 Puzzle - 프로젝트 기획 및 협업 플랫폼 🧩
> [2024-2 한림대학교 소프트웨어캡스톤디자인]
![image](https://github.com/user-attachments/assets/a66fec23-bcfe-480c-abf2-5e0bd5836f15)

### 🔗 배포 링크
https://spread-puzzle.io/

## 🚀 프로젝트 소개
Puzzle은 팀 프로젝트의 기획 및 설계 과정을 혁신적으로 개선하는 협업 플랫폼입니다. 체계적인 가이드라인과 실시간 협업 도구를 통해 프로젝트 관리의 새로운 패러다임을 제시합니다.

본 프로젝트는 **팀 프로젝트를 위한 아이디어 및 설계 가이드라인 제공 플랫폼** 개발을 통해 다음을 달성하는 것을 목표로 합니다:

- **프로젝트 기획 프로세스의 표준화**
  - 10단계로 구성된 체계적 기획 프로세스 제공.
  - 투표 기능을 통해 모든 팀원의 동의를 바탕으로 단계 전환 가능.

- **창의적 협업 환경 구축**
  - 실시간 시각적 캔버스와 협업 도구 제공.
  - 즉각적인 피드백을 지원하는 협업 환경 조성.

- **효율적인 문서화 및 관리 시스템 구현**
  - AI를 활용한 회의 내용 요약 및 관리.
  - 표준화된 Markdown 문서 자동 생성.


## 👥 Team : Spread
|FE | FE | BE |
| :---: | :---: | :---: |
| 김태호 | 김대성 | 김선우 |
|  <img width="130px" src="https://avatars.githubusercontent.com/u/126559845?v=4" /> |  <img width="130px" src="https://avatars.githubusercontent.com/u/131854855?v=4" /> |  <img width="130px" src="https://avatars.githubusercontent.com/u/114386406?v=4" /> |
| 한림대학교 | 한림대학교 | 한림대학교|
| 빅데이터전공 3학년 | 콘텐츠IT전공 4학년 | 빅데이터 3학년|
| [@low](https://github.com/ho8ae)|[@丸内大成](https://github.com/KimMaru10)|[@kimxxunu](https://github.com/Kimxxunu)|

## 📚 Tech Stacks


## [FronteEnd Repo](#)
<img src="https://img.shields.io/badge/html5-E34F26?style=for-the-badge&logo=html5&logoColor=white"><img src="https://img.shields.io/badge/Css-1572B6?style=for-the-badge&logo=Css&logoColor=white"><img src="https://img.shields.io/badge/typescript-3178C6?style=for-the-badge&logo=typescript&logoColor=white"><img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=React&logoColor=white">
<img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=Next.js&logoColor=white"><img src="https://img.shields.io/badge/tailwindcss-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white">



## [BackEnd Repo](#)
<img src="https://img.shields.io/badge/nestjs-E0234E?style=for-the-badge&logo=nestjs&logoColor=white"><img src="https://img.shields.io/badge/mongodb-47A248?style=for-the-badge&logo=mongodb&logoColor=white"><img src="https://img.shields.io/badge/swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=purple"><img src="https://img.shields.io/badge/docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"><img src="https://img.shields.io/badge/nginx-009639?style=for-the-badge&logo=nginx&logoColor=white"><img src="https://img.shields.io/badge/amazonwebservices-232F3E?style=for-the-badge&logo=amazonwebservices&logoColor=white">

## [AI](#)
<img src="https://img.shields.io/badge/python-3776AB?style=for-the-badge&logo=python&logoColor=white"><img src="https://img.shields.io/badge/openai-412991?style=for-the-badge&logo=mongodb&logoColor=white"><img src="https://img.shields.io/badge/langchain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white"><img src="https://img.shields.io/badge/fastapi-009688?style=for-the-badge&logo=fastapi&logoColor=white">

## [Design](#)
<img src="https://img.shields.io/badge/figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white"><img src="https://img.shields.io/badge/adobephotoshop-31A8FF?style=for-the-badge&logo=adobephotoshop&logoColor=white"><img src="https://img.shields.io/badge/adobeillustrator-FF9A00?style=for-the-badge&logo=adobeillustrator&logoColor=white">

## 💻 프로젝트 기능 소개
### 주요 기능

### Common Layout
| 라이브 커서 | 투표 | 프로세스 바 | 
|:--:|:--:|:--:|
| ![라이브커서](https://github.com/user-attachments/assets/1c923399-8006-4d01-96de-01704f4c67a4) | ![투표](https://github.com/user-attachments/assets/6aea4943-6a49-4ecc-93b6-22c6d4f156bd) | ![프로세스바](https://github.com/user-attachments/assets/48fd176c-7f45-47ff-8caa-f996a946c9e7)

| 접속자 | 타이머 | 오디오 플레이어 | 
|:--:|:--:|:--:|
 | ![접속자](https://github.com/user-attachments/assets/c3f64150-3d13-410e-85ce-7b24b8f733b7) | ![타이머](https://github.com/user-attachments/assets/412c8ecf-54fd-4548-9732-d4ed0c4c499e) | ![오디오 플레이어](https://github.com/user-attachments/assets/786e0bea-9238-430f-9aee-0dda283f74e7) 
 

| 사운드 블럭 | 그룹콜 | 툴바 | 
|:--:|:--:|:--:|
| ![사운드 블럭](https://github.com/user-attachments/assets/f0eeb171-8dd9-47ad-8317-42b3114ad714) | ![그룹콜](https://github.com/user-attachments/assets/0aef2ad8-303a-4ee3-951f-f3978f704ceb) | ![툴바](https://github.com/user-attachments/assets/116859de-d59a-419a-aa1b-50db88248900)


## 🚀 프로세스 10단계 소개

Puzzle은 체계적인 프로젝트 기획을 위해 아래와 같은 **10단계 프로세스**를 제공합니다. 각 단계는 명확한 목표와 도구를 지원하며, 모든 팀원이 동의해야 다음 단계로 넘어갈 수 있도록 설계되었습니다.

### 1️⃣ 아이스 브레이킹
- 👋 **팀원 간 친밀도 형성**  
  팀원들과 친밀도를 높이고, 팀 리더(호스트)를 선정합니다.
- 🌟 협업을 위한 팀 분위기를 조성합니다.

---

### 2️⃣ 비전 찾기
- 🎯 **프로젝트의 목표 설정**  
  프로젝트의 방향성과 목표를 명확히 정립합니다.
- 💡 팀원들이 공유할 수 있는 비전과 가치를 수립합니다.

---

### 3️⃣ 주제 선정
- 🧠 **아이디어 교환**  
  팀원들이 자유롭게 아이디어를 제시하고 논의합니다.
- ✅ 실행 가능한 주제를 선정하여 구체화합니다.

---

### 4️⃣ 스프레드
- 🛠️ **아이디어 발전**  
  제안된 아이디어를 발전시키고 구체적인 실행 계획으로 정리합니다.
- 📊 우선순위를 설정하고 아이디어의 발전 가능성을 평가합니다.

---

### 5️⃣ 토론하기
- 💬 **실현 가능성 논의**  
  아이디어의 실현 가능성과 문제점을 중심으로 논의합니다.
- 🤝 팀원 간의 의견을 조율하며 실행 방안을 도출합니다.

---

### 6️⃣ 페르소나
- 👤 **목표 사용자 정의**  
  프로젝트의 대상 사용자를 구체적으로 정의합니다.
- 🔍 사용자 요구와 니즈를 분석하여 기획 방향을 사용자 중심으로 조정합니다.

---

### 7️⃣ 문제 해결
- 🧩 **문제 정의 및 분석**  
  목표 사용자가 겪는 문제를 정의하고 이를 해결할 방안을 모색합니다.
- 🚀 구체적인 해결 방안을 실행 가능한 형태로 정리합니다.

---

### 8️⃣ 사용자 스토리
- 📖 **사용자 관점 설계**  
  제공할 주요 기능과 사용자 경험을 설계합니다.
- 🌟 사용자 관점에서의 가치를 명확히 정의합니다.

---

### 9️⃣ 메뉴 트리
- 🌐 **서비스 구조 설계**  
  서비스의 페이지 구조와 기능 흐름을 체계적으로 설계합니다.
- 🗂️ 프로젝트의 아키텍처를 정립합니다.

---

### 🔟 결과 확인하기
- 📄 **문서 최종 점검**  
  README.md 형태로 회의 내용을 요약합니다.
- 📝 요구사항 명세서를 검토하며 프로젝트 결과물을 최종 점검합니다.

---

| 1단계 아이스브레이킹 | 2단계 비전 찾기 |
|:--:|:--:|
| ![팀 스프레드 - 발표ppt의 사본](https://github.com/user-attachments/assets/b8020af1-4e04-47c1-b5f1-5a66e9065225) | ![팀 스프레드 - 발표ppt의 사본 (1)](https://github.com/user-attachments/assets/4dc58e58-fbe7-40c3-94ee-afbc76764a2e) 

| 3단계 주세선정 | 4단계 스프레드 | 
|:--:|:--:|
| ![팀 스프레드 - 발표ppt의 사본 (2)](https://github.com/user-attachments/assets/e6766340-7c54-4e7c-8205-047e4261e385) | ![팀 스프레드 - 발표ppt의 사본 (3)](https://github.com/user-attachments/assets/4f615b6d-6c8e-4454-ae41-26470882b4bc) 

| 5단계 토론하기 | 6단계 페르소나 | 
|:--:|:--:|
| ![팀 스프레드 - 발표ppt의 사본 (4)](https://github.com/user-attachments/assets/3b38613a-8652-4df0-93ff-c2043e4d5e3e) | ![팀 스프레드 - 발표ppt의 사본 (5)](https://github.com/user-attachments/assets/e23e8db8-ffcb-4380-a6e0-7918ec8e1b21)

| 7단계 문제 해결 | 8단계 사용자 스토리 | 
|:--:|:--:|
| ![팀 스프레드 - 발표ppt의 사본 (6)](https://github.com/user-attachments/assets/2800d6df-6e04-4e70-a2ec-8bf1d9298a66) | ![팀 스프레드 - 발표ppt의 사본 (7)](https://github.com/user-attachments/assets/5002793d-8ff2-43c7-a885-d48a0a92c4c3)

| 9단계 메뉴 트리 | 10단계 결과 확인하기 | 
|:--:|:--:|
| ![팀 스프레드 - 발표ppt의 사본 (8)](https://github.com/user-attachments/assets/1dbec64a-555a-4e38-8318-7abbde2d5dba) | ![팀 스프레드 - 발표ppt의 사본 (9)](https://github.com/user-attachments/assets/5191c4ee-b6bf-49a1-93fe-585a0cd80a30)

각 단계는 **실시간 협업 도구**와 **체계적인 가이드라인**을 통해 진행되며, 팀원 간의 민주적 의사결정을 지원합니다. Puzzle의 단계적 프로세스는 프로젝트 기획의 체계성과 효율성을 극대화합니다.


# 🧰 프로젝트 전체 아키텍쳐
### 전체 아키텍쳐
![전체 아키텍쳐_v2](https://github.com/user-attachments/assets/0f9c7bc1-1883-4e5b-9ba5-ade763aa78d5)


# ❗ 주요 프로세스
| 실시간 통신 프로세스 | 데이터 분석 프로세스 | 
|:--:|:--:|
|![실시간 통신](https://github.com/user-attachments/assets/03ffee34-d1be-4b0d-8537-c06b4e643911) | ![데이터 분석](https://github.com/user-attachments/assets/33b3940c-8f20-4d73-9ae8-469c76d74727)


<br>

# 🎥 프로젝트 시연 영상

### 아래의 이미지를 클릭하면 시연 영상을 확일 할 수 있습니다. 
[![Video Label](http://img.youtube.com/vi/YrecgU2v1b8/0.jpg)](https://youtu.be/YrecgU2v1b8)

<br>

##  📎 깃 주소
[- 팀 깃허브 주소](https://github.com/Team-SPREAD) <br>

[- AI 기허브 레포](https://github.com/Team-SPREAD/Puzzle_AI)   
   

